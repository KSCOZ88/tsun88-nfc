const {cors,stripeSession,verifyPaidSession,orderToken,deliver}=require('../server/stripe-orders');
module.exports=async function handler(req,res){
  ...
  try{
    ...
    const session=await stripeSession(String(body.sessionId||'')),order=body.order||{};const expected=verifyPaidSession(session,order);
    ...
    return res.status(200).json({paid:true,amountTotal:expected.totalCents,currency:'hkd',paymentIntentId:order.paymentTransactionId,orderToken:orderToken(session,order),mailSent:delivery.customerSent===true,delivery});
